import { DENSITY_LIMIT, LANE_COUNT, MAX_LANES, MIN_LANES } from '@/shared/config/constants';
...
/** Lane bars between two windows. */
const CIRCLE_COOLDOWN_BARS = 2;
...
function decide(seed: number, phrase: number, step: number, salt: number): number {
 ...
 * 4 lanes; the previous count is kept with probability 0.4 when the new pool allows it.
 */
export function planSections(bars: Bar[], variation: boolean, random: () => number, energetic = isEnergetic(bars)): SectionTuple[] {
  ...
  for (let p = 0; p * block < bars.length; p++) {
    ...
    else if (pool.includes(prevLanes) && random() < KEEP_LANES_CHANCE) lanes = prevLanes;
    else {
...
const gridBonus = (step: number): number => (step % 4 === 0 ? 0.15 : step % 2 === 0 ? 0.05 : -0.12) + (step % 8 === 0 ? 0.03 : 0);
...
export function composeChart(analysis: SongAnalysis, opts: ComposeOptions = {}): ChartLevel {
  ...
  const sections = planSections(bars, opts.laneVariation ?? true, random, energetic);
  ...
  const phrases: PhrasePattern[] = phrasesRaw.map((phraseBars, index) => {
    ...
    return { index, bars: phraseBars, profile, steps, accents, salMax, intense };
  });
  ...
  for (const bar of bars) {
    ...
    placed.sort((a, b) => a - b);
    for (const step of placed) events.push({ si: bar.start + step, bar, step, size: 1, hold: 0, kind: null, taps: 0, accent: phrase.accents.has(step) });
    if (rollFrom >= 0 && rollFrom < bar.slots.length) {
      const len = bar.slots.length - rollFrom;
      events.push({ si: bar.start + rollFrom, bar, step: rollFrom, size: 1, hold: len, kind: 'roll', taps: Math.max(ROLL_MIN_TAPS, Math.min(rollTaps, len)), accent: false });
    }
  }
  ...
  for (const bar of bars) {
    ...
    for (const wb of windowBars) {
      for (const step of phrase.steps) {
        if (!sounds(phrase, wb, step)) continue;
        hits.push({ si: wb.start + step, bar: wb, score: sal[wb.start + step] + gridBonus(step) });
      }
    }
    ...
    for (const c of chosen) events.push({ si: c.si, bar: c.bar, step: c.si - c.bar.start, size: 1, hold: 0, kind: 'circle', taps: 0, accent: false });
  }
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (slot.sustain >= 2 && melodic && budget > 0 && activeHoldEnds.length < MAX_ACTIVE_HOLDS && decide(seed, phraseIdx, ev.step, 2) < HOLD_CHANCE[intensity]) {
...
function eventsToTuples(events: readonly Event[], slots: readonly Slot[]): NoteTuple[] {
  const out: NoteTuple[] = [];
...
export function rateStars(notes: readonly NoteTuple[], bpm = 120, sections?: readonly SectionTuple[]): number {
  ...
  const raw =
    avgNps * STAR_W.nps +
    share16 * STAR_W.sixteenths +
    Math.max(0, peak - avgNps) * STAR_W.peak +
    wide * STAR_W.wide +
    special * STAR_W.special +
    chords * STAR_W.chords +
    tempo * STAR_W.tempo +
    STAR_W.offset;
