...
describe("trackBeats", () => {
  it("locks onto every click within 30 ms and finds the downbeat", () => {
    ...
    const beats = trackBeats(det.flux, det.hopSeconds, est.bpm).map(
      det.frameTime,
    );
    expect(Math.abs(beats.length - times.length)).toBeLessThanOrEqual(2);
    let matched = 0;
    for (const b of beats)
      if (times.some((t) => Math.abs(t - b) <= 0.03)) matched++;
    ...
    const nearestTruthIdx = times.findIndex(
      (t) => Math.abs(t - firstDownbeat) <= 0.03,
    );
...
describe("analyzeSong", () => {
  ...
  it("builds a 16-step grid per bar, starting on a downbeat, covering the track", () => {
    ...
    expect(analysis.beats[analysis.beats.length - 1]).toBeGreaterThanOrEqual(
      times[times.length - 1] - 0.03,
    );
    for (let i = 1; i < analysis.slots.length; i++)
      expect(analysis.slots[i].time).toBeGreaterThan(
        analysis.slots[i - 1].time,
      );
    const bar1 = analysis.slots.filter((s) => s.bar === 1);
    expect(bar1.length).toBe(STEPS_PER_BAR);
    const downbeatStrength = analysis.slots
      .filter((s) => s.step === 0 && s.time > 1 && s.time < 30)
      .map((s) => s.strength);
    expect(Math.min(...downbeatStrength)).toBeGreaterThan(0.5);
  });

  it("marks clicks as strong slots and silence as weak", () => {
    const onBeat = analysis.slots.filter(
      (s) => s.step % 4 === 0 && s.time > 1 && s.time < 30,
    );
    const offBeat = analysis.slots.filter(
      (s) => s.step % 4 === 2 && s.time > 1 && s.time < 30,
    );
    const avg = (xs: Slot[]) =>
      xs.reduce((a, s) => a + s.strength, 0) / xs.length;
...
function fakeAnalysis(
  bars: number,
  pattern: (bar: number, step: number) => Partial<Slot>,
): SongAnalysis {
  const slots: Slot[] = [];
  const beats: number[] = [];
...
function maxFingers(notes: readonly NoteTuple[]): number {
  ...
  for (const t of starts) {
    ...
    const holding = notes.filter(
      (n) => isHoldType(n) && n[0] < t && n[0] + (n[2] as number) > t + 1e-6,
    ).length;
...
const drumLoop = (bar: number, step: number): Partial<Slot> => {
  ...
  if (step % 2 === 0)
    return { strength: intense ? 0.8 : 0.45, low: 0.1, mid: 0.3, high: 0.6 };
...
const fillLoop = (bar: number, step: number): Partial<Slot> => {
  if (bar % 4 === 3 && step >= 12)
    return { strength: 0.95, low: 0.4, mid: 0.4, high: 0.2 };
...
describe("composeChart", () => {
  ...
  it("puts every note exactly on a grid slot and never needs more than two fingers", () => {
    ...
    expect(
      maxFingers(composeChart(fakeAnalysis(24, sustainedLoop)).notes),
    ).toBeLessThanOrEqual(2);
  });

  it("respects the density limit, lane rules and chord validity", () => {
    const { notes, sections } = chart;
    const lanesAt = (t: number) =>
      sections!.filter((s) => s[0] <= t + 1e-9).pop()![1];
    const starts = [...new Set(notes.map((n) => n[0]))];
    for (const t of starts)
      expect(
        starts.filter((s) => s >= t && s < t + 1).length,
      ).toBeLessThanOrEqual(DENSITY_LIMIT);
    const events = starts.map((t) =>
      notes.filter((n) => n[0] === t).map((n) => n[1]),
    );
    for (let i = 2; i < events.length; i++)
      for (const lane of events[i])
        expect(
          events[i - 1].includes(lane) && events[i - 2].includes(lane),
        ).toBe(false);
    for (const t of starts) {
  ...
  it("changes the lane count per phrase and keeps every note inside its section", () => {
    ...
    expect(composeChart(analysis, { laneVariation: false }).sections).toEqual([
      [0, 4],
    ]);
  });
  ...
  it("leaves two beats of silence before every lane-count change", () => {
    ...
    for (let i = 1; i < sections.length; i++) {
      ...
      const before = chart.notes.filter(
        (n) =>
          n[0] < t &&
          n[0] + (n.length >= 3 ? (n[2] as number) : 0) > t - 1 + 1e-6,
      );
  ...
  it("reproduces the phrase figure note-for-note in every bar and makes accents chords", () => {
    ...
    const barOf = (t: number) =>
      Math.floor(Math.round(t / 0.125) / STEPS_PER_BAR);
    for (let b = 0; b < 16; b++) {
      const own = notes.filter((n) => barOf(n[0]) === b);
      expect(
        [...new Set(own.map((n) => stepOf(n[0])))].sort((x, y) => x - y),
      ).toEqual([0, 2, 4, 6, 8, 10]);
      for (const step of [4, 10])
        expect(own.filter((n) => stepOf(n[0]) === step).length).toBe(2);
      for (const step of [0, 2, 6, 8])
        expect(own.filter((n) => stepOf(n[0]) === step).length).toBe(1);
    }
  ...
  it("allows sixteenth pairs in intense bars when the pattern has them, an eighth apart otherwise", () => {
    ...
    expect(
      [...new Set(early.map((n) => stepOf(n[0])))].sort((x, y) => x - y),
    ).toEqual([0, 4, 7, 8, 12, 14]);
    expect(maxFingers(notes)).toBeLessThanOrEqual(2);
  ...
  it("turns drum fills into rolls", () => {
    const rolls = composeChart(fakeAnalysis(16, fillLoop)).notes.filter(
      (n) => n[3] === "roll",
    );
  ...
  it("never starts a note in a lane that is still being held", () => {
    ...
    for (const h of holds) {
      ...
      const clash = notes.some(
        (n) => n !== h && n[1] === h[1] && n[0] > h[0] && n[0] < end - 1e-6,
      );
  ...
  it("gives quiet intros sparse notes but leaves true silence empty", () => {
    ...
    expect(notes.filter((n) => n[0] >= 8 && n[0] < 16).length).toBeGreaterThan(
      0,
    );
  });

  it("places alternating slow / heart spell notes as plain taps", () => {
    const spells = chart.notes.filter(
      (n) => n[3] === "slow" || n[3] === "heart",
    );
  ...
  it("is deterministic for the same seed", () => {
    expect(composeChart(analysis, { seed: 7 })).toEqual(
      composeChart(analysis, { seed: 7 }),
    );
  });
  ...
  it("prefers templates that match the hits", () => {
    ...
    const q = TEMPLATES.find((t) => t.id === "quarter")!;
    const e = TEMPLATES.find((t) => t.id === "eighth")!;
    const r = TEMPLATES.find((t) => t.id === "rest")!;
    expect(scoreTemplate(q, quarters, 1, false)).toBeGreaterThan(
      scoreTemplate(e, quarters, 1, false),
    );
    expect(scoreTemplate(q, quarters, 1, false)).toBeGreaterThan(
      scoreTemplate(r, quarters, 1, false),
    );
  });
});
  ...
  it("cycles 8 positions across the field without two equal in a row", () => {
    for (let n = 2; n <= 6; n++) {
      ...
      for (let i = 0; i < spread.length; i++)
        expect(spread[i]).not.toBe(spread[(i + 1) % spread.length]);
...
describe("rateStars", () => {
  const taps = (nps: number, seconds: number): NoteTuple[] =>
    Array.from({ length: Math.round(nps * seconds) }, (_, i) => [
      Math.round((i / nps) * 1000) / 1000,
      i % 4,
    ]);
  ...
  it("grows with density, width, mechanics and tempo", () => {
    ...
    expect(rateStars(base, 120, [[0, 6]])).toBeGreaterThanOrEqual(
      rateStars(base, 120, [[0, 3]]),
    );
    const withRolls = base.map((n, i): NoteTuple =>
      i % 4 === 0 ? [n[0], n[1], 0.4, "roll", 3] : n,
    );
