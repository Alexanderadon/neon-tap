...
/** Credit crystals earned in a run (non-positive amounts are ignored). */
 ...
 * the track is already owned or the balance is short. Idempotent per id.
 */
export function purchaseTrack(save: SaveData, trackId: string, price: number): { save: SaveData; ok: boolean; reason?: PurchaseFailure } {
  ...
  return { save: { ...save, crystals: save.crystals - cost, purchased: [...save.purchased, trackId] }, ok: true };
}
