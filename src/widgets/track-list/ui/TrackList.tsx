...
import { needFor, useCatalogState } from '../model/useCatalogState';
...
export const TrackList = forwardRef<HTMLElement, Props>(function TrackList({ onRecords, selectedId, onSelect }, ref) {
  ...
  const tap = (t: TrackMeta, locked: boolean) => {
    if (t.id === selectedId && !locked) {
      sfxUi();
      void play(t.id);
      return;
    }
...
interface CardProps {
  ...
  selected: boolean;
  busy: boolean;
  onTap: (track: TrackMeta, locked: boolean) => void;
  onRecords?: (track: TrackRef) => void;
}

function ReelCard({ index, track, best, need, daily, selected, busy, onTap, onRecords }: CardProps) {
  ...
  const cls = ['reel-card', selected && 'is-selected', locked && 'is-locked', daily && 'is-daily'].filter(Boolean).join(' ');
  return (
