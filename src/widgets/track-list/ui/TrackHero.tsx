...
export function TrackHero({ trackId, top, onRecords }: Props) {
  ...
  const need = needFor(state, track.id);
  const locked = need > 0;
      ...
      <div className="hero-cta">
        ...
        <div className="hero-play-wrap">
          <button
            ...
            aria-label={locked ? fmt(dict.menuCardLocked, { title: track.title, n: need }) : fmt(dict.menuHeroPlayAria, { title: track.title })}
          >
            ...
            <span className="hero-play-face">
              {locked ? <LockIcon size={34} /> : <PlayIcon size={44} />}
              <span className="hero-play-text micro">{locked ? dict.locked : dict.menuHeroPlay}</span>
            </span>
          </button>
          {locked && <div className="hero-unlock mono">{fmt(dict.menuHeroUnlock, { need, have: state.stars })}</div>}
        </div>
      </div>
    </section>
  );
