...
export function SongDropZone() {
  ...
  const play = (difficulty: Difficulty) => {
    ...
    startSession(state.song.chart, difficulty, 'custom', state.song.audioBuffer);
    navigate('game');
  };
  ...
  if (state.kind === 'ready') {
    ...
    return (
      <div className="dropzone dropzone-ready">
        ...
        <div className="dropzone-choose">{dict.customChoose}</div>
        <div className="dropzone-diffs">
          {DIFFICULTIES.map((d) => (
            <Button key={d} size="lg" onClick={() => play(d)}>
              {dict.difficulty[d]} · ★ {song.chart.charts[d].stars}
              <small>
                {song.chart.charts[d].notes.length} · {(song.chart.charts[d].notes.length / song.chart.duration).toFixed(1)} {dict.notesPerSec}
              </small>
            </Button>
          ))}
        </div>
