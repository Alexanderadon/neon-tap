...
export type AnalysisMessage =
  ...
  | { type: 'done'; bpm: number; offset: number; beats: number[]; onsets: number; confidence: number; chart: ChartLevel }
...
self.onmessage = (e: MessageEvent<AnalysisRequest>) => {
  try {
    ...
    post({ type: 'done', bpm: analysis.bpm, offset: analysis.beats[0] ?? 0, beats: analysis.beats, onsets: analysis.onsetCount, confidence: analysis.confidence, chart });
  } catch (err) {
