...
- **Tutorial** — an interactive first run (after calibration, once; a "Tutorial" button in the menu): a hand-written chart `public/charts/tutorial.json` on the beat grid of Apparatus Overlord walks through tap → all lanes → hold → slide → roll → circles → lane change → finale; step captions with animated SVG icons and hints for keyboard and fingers, no hearts and no fail (`features/tutorial`, `widgets/tutorial-overlay`, `pages/tutorial`).
