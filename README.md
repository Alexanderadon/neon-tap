...
Audio output latency: desktop 20–60 ms, Android 80–200 ms, Bluetooth up to 300 ms. In calibration (opened from settings) a 120 BPM metronome is scheduled by a look-ahead scheduler on the audio clock; the player taps 16 times; the median deviation (robust against outliers from the first taps) becomes `audioOffsetMs`. A manual −300…+300 ms slider is available in settings.
